...
%% our case we have ready access to the line, but proven operation
%% sequences for common operations.
%% %-------------------------------------------------------------------

%% Send symbolic operations generated by cli_edlin.erl or other.
%% We need the characters after the cursor, before these operations are applied passed in here because some ops require us to re-write the whole part of the line after the cursor

% -spec send_ops(ops(), list(char()), #term{}) -> #term{}.
send_ops(Ops, #term{} = Term) ->
    lists:mapfoldl(fun(Op, #term{} = T) -> send_op(Op, T) end, Term, Ops).

send_op({move_rel, N}, T)                  -> move_rel(N, T);
send_op({put_chars, unicode, Chars}, T)    -> put_chars(Chars, T);
send_op({insert_chars, unicode, Chars, Aft}, T) -> ins_chars(Chars, Aft, T);
send_op({delete_chars, N, Aft}, T)              -> del_chars(N, Aft, T);
send_op(crnl, T) ->
    Pos = size(T#term.prompt),
    {["\r\n[ok]\r\n",T#term.prompt], T#term{lpos = Pos, llen = Pos}};
send_op(beep, T)              -> io:format("beep\r\n"), {[], T}.
